...
export default async function handler(req, res) {
  ...
  try {
    ...
    // All unique visitor locations for the world map (one entry per IP with coords)
    // This avoids the map being limited to only the last 20 page views
    const seenIpsForMap = new Set();
    const allVisitorLocations = [];
    for (let i = pageViews.length - 1; i >= 0; i--) {
      const v = pageViews[i];
      const ip = v?.ip || v?.ipAddress;
      if (ip && !seenIpsForMap.has(ip) && typeof v.latitude === 'number' && typeof v.longitude === 'number') {
        seenIpsForMap.add(ip);
        allVisitorLocations.push(v);
      }
    }
