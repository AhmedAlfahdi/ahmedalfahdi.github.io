...
# Example: TNT at Chapman-Jouguet (CJ) state
P_CJ = jwl_pressure(V=1.0, E=3.681, 
                     A=373.77, B=3.747, 
                     R1=4.15, R2=0.90, omega=0.35)
print(f"TNT CJ Pressure: {P_CJ:.2f} GPa")

# Try different volumes to see pressure variation
print(f"\\nPressure vs Volume:")
for V in [0.5, 0.75, 1.0, 1.25, 1.5, 2.0]:
    P = jwl_pressure(V, E=3.681, A=373.77, B=3.747, 
                      R1=4.15, R2=0.90, omega=0.35)
    print(f"  V = {V:.2f} → P = {P:.2f} GPa")`}
/>
...
# Example: Sound speed at Chapman-Jouguet (CJ) state
...
for V in [0.8, 1.0, 1.5, 2.0, 3.0]:
    # First calculate pressure at this volume
    P = (373.77 * (1 - 0.35/(4.15*V)) * math.exp(-4.15*V) + 
         3.747 * (1 - 0.35/(0.90*V)) * math.exp(-0.90*V) + 
         0.35 * 3.681 / V)
    
    c = jwl_sound_speed(V, E=3.681, P=P, A=373.77, B=3.747, 
                        R1=4.15, R2=0.90, omega=0.35)
    print(f"  {V:<10.2f} {P:<15.2f} {c:<20.2f}")`}
/>
