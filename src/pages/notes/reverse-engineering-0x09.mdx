...
> **💡 Hint:** The format `DX:AX` shows that the MSB (Most Significant Byte) is stored in `DX` and LSB is stored in `AX`
...
> **⚠️ Error:** When trying to compile this using `build_x86` it gives a fatal error. That is because the compiler (assembler) needs to be `x64`. Also, the `dump-regs` function is written for `x86` only.
...
| ----------- | ------------ | ---------------------------------- | ---------------------------- |
| `mul`       | Unsigned     | Always ≥ 0 (non-negative)          | Product treated as unsigned  |
...
graph LR
    ...
    B --> B1[AL × operand → AX]
    C --> C1[AX × operand → DX:AX]
    D --> D1[EAX × operand → EDX:EAX]
    E --> E1[RAX × operand → RDX:RAX]
...
<PDFViewer src="/pdfs/20000096503.pdf" title="Assembly Reference" height="600px" />
